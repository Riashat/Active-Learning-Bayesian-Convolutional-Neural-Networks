...
Bald = np.load('Accuracy_Q5_N1000.npy')
...
pl.subplot(2, 1, 1)
plt.plot(Queries, Bald, color="blue", linewidth=2.0, marker='^', label=r"\textbf{Dropout Bayesian Active Learning by Disagreement}" )
plt.xlabel(r'\textbf{Number of Queries}')
plt.ylabel(r'\textbf{Accuracy on Test Set}')
plt.title(r'\textbf{Dropout BALD Query Point Acquisition, upto 1000 Labelled Samples}')
plt.grid()
plt.legend(loc = 4)
...
plt.title(r'\textbf{Dropout Bald Model Fitting with Query Point Acquisition}')
plt.grid()
plt.legend(loc = 4)
