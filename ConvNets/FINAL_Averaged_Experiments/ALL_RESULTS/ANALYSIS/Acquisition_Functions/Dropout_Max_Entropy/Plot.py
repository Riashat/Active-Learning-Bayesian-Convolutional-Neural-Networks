...
Dropout_Max_Entropy = np.load('Accuracy_Q5_N1000.npy')
...
pl.subplot(2, 1, 1)
plt.plot(Queries, Dropout_Max_Entropy, color="blue", linewidth=2.0, marker='^', label=r"\textbf{Dropout Maximum Entropy Acquisition Function}" )
plt.xlabel(r'\textbf{Number of Queries}')
plt.ylabel(r'\textbf{Accuracy on Test Set}')
plt.title(r'\textbf{Dropout Max Entropy Query Point Acquisition, upto 1000 Labelled Samples}')
plt.grid()
plt.legend(loc = 4)
...
plt.title(r'\textbf{Dropout Max Entropy Model Fitting with Query Point Acquisition}')
plt.grid()
plt.legend(loc = 4)
