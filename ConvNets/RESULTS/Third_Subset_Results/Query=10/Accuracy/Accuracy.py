...
plt.plot(Queries, Random, color="magenta",linewidth=3.0, marker='x', label=r"\textbf{Random Acquisition}" )
plt.plot(Queries, Variation, color="orange",linewidth=3.0, marker='x', label=r"\textbf{Dropout - Variation Ratio}" )
# plt.plot(Queries, Highest_Entropy, color="green",linewidth=3.0, marker='x', label=r"\textbf{Max Entropy}" )



plt.xlabel(r'\textbf{Number of Queries}')
plt.ylabel(r'\textbf{Accuracy on Test Set}')
plt.title(r'\textbf{Comparing Acquisition Functions - 10 Queries per Acquisition, 1000 Labelled Samples}')
plt.grid()
# Set x limits
