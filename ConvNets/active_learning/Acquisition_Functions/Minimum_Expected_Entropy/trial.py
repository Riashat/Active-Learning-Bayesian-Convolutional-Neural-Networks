...
for i in range(iterations):			# for each round t
	...
	for pool_point in range(2):# X_Pool.shape[0]			# for each unlabelled image
		
		for pool_label in range(1): # nb_classes			# for each possible class label
			...
			for rm_pool_point in range(1):	#Remaining_Pool_Points.shape[0]
				for pool_class in range(1):	#nb_classes
